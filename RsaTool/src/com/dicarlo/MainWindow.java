...
public class MainWindow extends JFrame {
		...
		if (buttonFileInD == null) {
			...
			buttonFileInD.addActionListener((new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent evt) {
					...
					fileChooser.addChoosableFileFilter(new FileNameExtensionFilter("XML Ciphered files", "xml", "ciphered"));
					if (fileChooser.showOpenDialog(parent) == JFileChooser.APPROVE_OPTION) {
					  File file = fileChooser.getSelectedFile();
						if (file != null){
						  if(file.exists()){
	...
	private void encryptFile(){
	  new Thread() {
		@Override public void run () {
		  ...
		  String dateFormatted=sdf.format(new java.util.Date(System.currentTimeMillis()));
			  ...
			  while ((string = in.readLine()) != null){ 
					...
					try{
                        ...
                        if(!listSerial.contains(lineserial)){
						  ...
						  outputWriter.write(getObj2XmlStr(kp,i).getBytes("UTF8"));
						  outputWriter.flush();
						  listSerial.add(lineserial);
                        }
                        else{
                        	logger.error("Error: element("+(i+1)+") file("+srcFileName+") serial("+lineserial+") duplicate");
                        }
					}
					catch(Exception e){
						e.printStackTrace();
						logger.error("Error: element("+(i+1)+") file("+srcFileName+")",e);
					}
	...
	private byte[] keyGetByteCiphered(String value){
		try {
			...
			//byte[]byteSeed = cipher.doFinal(formatKey(value.getBytes("UTF8")));
	...
	private synchronized String[] getSerialKey(String[] stringArray) throws Exception{
		...
		if(stringArray.length>0){
		  for(int i=0;i<stringArray.length;i++){
			...
			if(hexdecimal.matcher(value.toUpperCase()).matches()&&"".equals(serial)&&value.length()<=20) {
				serial=value;
			}
		  }	
	...
	private synchronized String getObj2XmlStr(KeyPackage kp,long i) {
        StringBuffer sb = new StringBuffer();
			...
			String sAlgoType = kp.getKey().getAlgorithmParameters().getAlgoType() == Constants.ALGO_TYPE_HOTP ? "Counter" : "Time";
			
			sb.append("<"+sAlgoType+"><PlainValue>"+kp.getKey().getData().getCounter()+"</PlainValue></"+sAlgoType+">\n");
